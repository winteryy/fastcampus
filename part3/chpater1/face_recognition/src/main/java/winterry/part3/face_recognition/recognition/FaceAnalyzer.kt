package winterry.part3.face_recognition.recognition
...
internal class FaceAnalyzer(
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        ...
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.UnDetect) {
                ...
                && (face.leftEyeOpenProbability ?: 0F) > EYE_OPENED_VALUE
                && (face.rightEyeOpenProbability ?: 0F) < EYE_CLOSED_VALUE
            ) {
                detectStatus = FaceAnalyzerStatus.LeftWink
                listener?.detectProgress(50F, "오른쪽 눈을 깜빡여주세요.")
            } else if (detectStatus == FaceAnalyzerStatus.LeftWink
                && (face.leftEyeOpenProbability ?: 0F) < EYE_CLOSED_VALUE
                && (face.rightEyeOpenProbability ?: 0F) > EYE_OPENED_VALUE
            ) {
    ...
    companion object {
        private const val EYE_CLOSED_VALUE = 0.035F
        private const val EYE_OPENED_VALUE = 0.35F
